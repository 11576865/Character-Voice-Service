...
- 保存并打开书籍后，在“生成前预览本章声音安排”选择章节，按当前旁白、参考和连续规划设置查看每段拟用的角色、参考及选择依据。只修改需要纠正的段落并保存，再重新预览；整书生成与预览使用同一套选择逻辑。跨角色时未指定参考会使用新角色的默认参考，或在顶部选“自动”时为新角色自动选择。
...
- `/epub-prototype` 是独立的 foliate-js EPUB 排版映射验证页：可打开本地 EPUB、查看章节路径匹配、尝试目录跳转和逐段定位。页面先检查浏览器能否载入排版所需的 `blob:` 框架；失败时会说明浏览器兼容问题，载入缓慢时提示超时。正式 Reader 仍使用现有文本视图，直到段落定位和高亮实测通过。
